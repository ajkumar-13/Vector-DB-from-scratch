...
> **TL;DR.** This post builds the durability layer of the database: a Write-Ahead Log that records every `Insert` and `Delete` to disk before touching in-memory state. You will design a self-describing binary entry format guarded by a CRC32 checksum, learn why appending sequentially beats random writes, and confront the `fsync` trade-off between durability and throughput. By the end you can survive a power failure and replay the log to rebuild exactly where you left off.
>
> **What you will build:**
> - A binary WAL entry format (CRC32 + length + `bincode` payload) that detects torn writes
> - An append-only `WriteAheadLog` writer using `BufWriter`, with a tunable `fsync` (group-commit) policy
> - A corruption-tolerant replay reader that stops cleanly at the first bad entry
> - A `VectorStore` that logs before mutating memory and recovers its full state on startup

---

## 1. Introduction: The "Memory Amnesia" Problem

This is where the project stops being merely fast and starts being safe. In [the previous post on memory mapping](../07-mmap/index.md), we built a high-speed read path with `mmap`; here we add the write path that guarantees nothing is lost when the process dies. Together they form the storage core the rest of the series builds on.

In [Post #7](../07-mmap/index.md), we built a high-speed storage engine using `mmap`. It's incredibly fast for reading massive datasets.
...
- **Forgetting that `write()` is not durable.** A successful `self.writer.write_all(...)` only moves bytes into the `BufWriter` buffer, and even after `flush()` they may sit in the OS page cache. Only `sync_all()` forces them to physical media. If you return `200 OK` to a client before any `sync`, you are promising durability you have not delivered.
- **Checksumming the wrong bytes.** The CRC must cover exactly the same bytes on write and on replay. Here we hash the little-endian length bytes followed by the payload. If the writer hashes `payload.len() as u32` but the reader hashes a different width, a different endianness, or omits the length entirely, every entry will appear corrupt. Keep the two `Hasher` sequences byte-for-byte identical.
- **Opening the file without append mode.** `OpenOptions::new().create(true).append(true)` is load-bearing. Drop `.append(true)` and writes land at offset zero, silently overwriting earlier entries; add `.truncate(true)` and you wipe the entire history on every restart. The whole safety argument rests on never moving the write cursor backward.
- **Erroring out instead of stopping at corruption.** A torn tail entry is the expected outcome of a crash, not a fatal error. Returning `Err` from `read_all` on the first bad CRC throws away all the valid history before it. Recover everything up to the first bad record, then `break`.
- **Syncing on every single write in the name of safety.** One `fsync` per insert caps you near a couple hundred writes per second. Batch with a group-commit policy (`append_with_policy`) and accept a bounded window of potential loss. "Sync per write" is a correctness choice, not a free default.
- **Updating memory before the log.** The Golden Rule is write-ahead for a reason: if you insert into the `HashMap` first and the append fails (disk full, I/O error), your in-memory state now diverges from what replay will reconstruct. Always append, check the result, then mutate memory.

---

## What to read next

- **[Crash Recovery: Replaying Logs and Restoring State](../09-crash-recovery/index.md)**: the WAL grows forever, so next we replay it on startup, checkpoint entries into immutable `mmap` segments, and safely truncate the log.

---

## Further reading

- The Rust Standard Library, [`std::fs::File::sync_all`](https://doc.rust-lang.org/std/fs/struct.File.html#method.sync_all) and `BufWriter` documentation: the precise durability semantics of flush versus sync.
- The `bincode` crate documentation ([docs.rs/bincode](https://docs.rs/bincode/)): the compact binary serialization format used for WAL payloads.
- The `crc32fast` crate documentation ([docs.rs/crc32fast](https://docs.rs/crc32fast/)): hardware-accelerated CRC32 checksums for corruption detection.
- Mohan et al., "ARIES: A Transaction Recovery Method Supporting Fine-Granularity Locking and Partial Rollbacks Using Write-Ahead Logging" (ACM TODS, 1992): the canonical treatment of write-ahead logging and crash recovery.
