...
> **TL;DR.** This post wires a production full-text engine, Tantivy, alongside your HNSW index so a single query can filter by metadata and rank by vector similarity. You will learn the sidecar pattern, how to keep IDs aligned across two engines, and how to turn filter results into an O(1) bitmask that HNSW traversal can consult on every hop. The result is pre-filtered hybrid search with perfect recall at roughly 3ms instead of 180ms for brute force.
>
> **What you will build:**
> - A `MetadataIndex` wrapper around Tantivy with an explicit `point_id` field bridging it to HNSW.
> - A `search_to_bitmask` path that converts filter matches into a `Vec<bool>` for fast-lane lookups.
> - A filter-aware `search_with_filter` in HNSW, plus an automatic brute-force fallback for low-selectivity filters.
> - A unified `HybridSearchEngine` API combining vectors and boolean metadata queries.

---

## 1. Introduction: Don't Reinvent the Wheel

Coming out of [Inverted Indexes Explained](../17-inverted-indexes/index.md), where we hand-built tokens, postings lists, and set intersections, this post is where the build stops simulating a search engine and starts embedding a real one next to the HNSW graph we have grown across the series.

In [Post #17](../17-inverted-indexes/index.md), we built a simple inverted index from scratch. It taught us the fundamentals: tokens, postings lists, and set intersections.
...
Now we modify the `HNSWIndex::search` method from [Post #15](../15-hnsw-impl-2/index.md) to accept an optional filter.
...
- **Forgetting to commit before you search.** Tantivy buffers writes in the `IndexWriter`. Until you call `commit()`, the reader (even with `ReloadPolicy::OnCommit`) sees nothing, so `search_to_bitmask` returns an all-false mask and every hybrid query comes back empty. If you index in a loop, commit once at the end rather than per document, but never skip it.
- **Reading fast fields from segment 0 only.** The simplified `search_to_bitmask` hardcodes `segment_reader(0)`. After enough writes Tantivy creates multiple segments, and a `doc_address` from segment 2 read against segment 0's fast field yields a wrong or out-of-bounds `point_id`. Always resolve the reader from `doc_address.segment_ord`, as in `search_to_bitmask_multi_segment`.
- **Treating Tantivy's `DocAddress` as the HNSW id.** The `(segment_ord, doc_id)` pair is internal and shifts on merges. The HNSW `PointId` is the source of truth, which is exactly why you store `point_id` as an `INDEXED | FAST | STORED` field and translate every match back through the fast field. Skipping that translation silently corrupts results.
- **Letting a restrictive filter disconnect the graph.** When a filter matches a tiny fraction of documents, greedy HNSW traversal can get stranded with no reachable matching neighbors and return too few results. Guard with a selectivity check (`search_hybrid`) and fall back to filtered brute force below roughly 1 percent, instead of blindly trusting the graph walk.
- **Allocating the bitmask with the wrong size.** `search_to_bitmask` takes `max_id`, and any `point_id >= max_id` is dropped by the bounds check. Pass `hnsw.num_points()` (the current count), not a stale constant, or you will silently filter out the newest documents.
- **Assuming deletes free space immediately.** `delete_term` only writes a tombstone. The document keeps occupying its segment until a merge runs, so disk usage and even some query cost persist after a "delete." Plan a merge policy rather than expecting `delete_by_point_id` to reclaim space.

---

## What to read next

- **[Query Planning: Vector-First vs Filter-First](../19-query-planning/index.md)**: build a cost-based optimizer that decides automatically when to pre-filter, when to post-filter, and when to brute force, generalizing the ad hoc selectivity check from this post.

---

## Further reading

- **Tantivy** (Quickwit / Paul Masurel): the Rust full-text search library used here. Crate docs at [https://docs.rs/tantivy](https://docs.rs/tantivy), source at [https://github.com/quickwit-oss/tantivy](https://github.com/quickwit-oss/tantivy).
- **Apache Lucene**: the Java search library that inspired Tantivy's segment, codec, and BM25 design. [https://lucene.apache.org](https://lucene.apache.org).
- **Roaring Bitmaps** (Chambi, Lemire, Kaser, Godin): compressed bitmap structure widely used for fast set membership in search engines. [https://roaringbitmap.org](https://roaringbitmap.org).
- **Efficient and Robust Approximate Nearest Neighbor Search Using Hierarchical Navigable Small World Graphs** (Malkov and Yashunin, 2018, IEEE TPAMI): the HNSW algorithm whose traversal we filter. [https://arxiv.org/abs/1603.09320](https://arxiv.org/abs/1603.09320).
