...
> **TL;DR.** Loading a file with `File::read_to_end` copies every byte into your process and dies when the dataset is larger than RAM. This post replaces that with memory mapping (`mmap`) via the `memmap2` crate, so the OS pages your segment files in on demand and treats a 100GB file like a plain `&[u8]`. You will build an `MmapSegment` that validates the binary header in place and hands out zero-copy `&[f32]` views over vectors, plus an iterator and a benchmark proving the difference.
>
> **What you will build:**
> - An `MmapSegment` type that memory-maps a segment file and validates its magic, version, and size against the mapped bytes
> - Zero-copy `get_vector` access that casts mapped bytes to `&[f32]` (the unsafe way, then the safe `bytemuck` way)
> - An `ExactSizeIterator` over vectors that never loads the whole file
> - A benchmark contrasting `read()` against `mmap` so you know when each one wins

## 1. Introduction: The RAM Limit Problem

This post sits in the storage layer of the build. In the [previous post](../06-binary-file-formats/index.md) we designed a custom binary segment layout (magic bytes, version, count, dimension, then packed `f32` data); here we stop reading those segments into a `Vec` and instead map them straight into virtual memory.

In [Post #6](../06-binary-file-formats/index.md), we designed a binary format to store millions of vectors. We wrote a `read_segment` function that opens the file, reads every byte, and parses it into a `Vec<Vector>` in memory.
...
- **Casting bytes to `&[f32]` from an unaligned offset.** Both `slice::from_raw_parts` and `bytemuck::cast_slice` assume 4-byte alignment for `f32`. The mapping base is page-aligned (4KB), so this only holds if your `data_start` is itself a multiple of 4. Our 16-byte header keeps the data region aligned; if you ever change the header size to a non-multiple of 4, the cast will panic (bytemuck) or be undefined behavior (raw pointers). Keep the header padded to a 4-byte boundary.
- **Trusting `count` and `dim` before validating the file size.** `expected_size = 16 + count * dim * 4` can overflow `usize` on a corrupt header, and a truncated file will let `get_vector` index past the mapping and trigger SIGBUS. Always validate `mmap.len() >= expected_size` at open time (as `MmapSegment::open` does) instead of discovering the problem on access.
- **Mutating a file while it is mapped.** If another process (or your own writer) truncates or shrinks a mapped file, touching the vanished pages crashes with SIGBUS, which Rust cannot catch. Treat mapped segments as immutable: write to new files and swap, never resize in place.
- **Accessing `mmap[i]` directly from an async task.** A page fault looks like a memory read but can block for milliseconds on disk I/O, stalling the Tokio executor and starving every other task on that thread. Move scans that walk cold pages into `spawn_blocking`.
- **Treating `mmap` as a universal speedup.** For a small file you fully scan once, `read()` is just as fast and far simpler, and the page-fault overhead of `mmap` can make random access slower per element. `mmap` wins for datasets larger than RAM, sparse access, and shared read-only files, not for everything.
- **Forgetting that `Mmap::map` is `unsafe` for a reason.** The `unsafe` block is not a formality you can suppress with a comment; it is your contract that no other process will invalidate the mapping. Document that invariant at every call site rather than copying the `// Safety:` line without thinking about who else writes the file.

---

## What to read next

- **[The Append-Only Log: Write-Ahead Log (WAL)](../08-wal/index.md)**: mapped segments are read-only, so new inserts need a durable staging area first; the WAL is where writes land before they are compacted into segment files.

---

## Further reading

- The `memmap2` crate documentation (docs.rs/memmap2): the API and safety contract for `Mmap`, `MmapOptions`, and `Advice` used throughout this post.
- The `bytemuck` crate documentation (docs.rs/bytemuck): the safe `cast_slice` helper and the `Pod`/`Zeroable` traits behind the zero-copy `&[f32]` view.
- The Rust Programming Language, chapter on unsafe Rust (doc.rust-lang.org/book/ch20-01-unsafe-rust.html): why raw pointer dereferences and `from_raw_parts` require `unsafe` and what invariants you are promising.
- mmap(2), Linux man pages (man7.org/linux/man-pages/man2/mmap.2.html): the underlying syscall, page-fault semantics, and the `madvise` hints surfaced as `Advice::Sequential` and friends.
