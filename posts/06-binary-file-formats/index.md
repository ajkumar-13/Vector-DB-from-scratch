...
> **TL;DR.** JSON is great for talking to humans, but it is a terrible way to store millions of vectors on disk. This post designs a compact, crash-aware binary segment format (`.vec`) with a fixed 16-byte header, magic bytes, a version field, and tightly packed Little Endian `f32` data. You will hand-write the serialization and deserialization code in Rust and learn why a fixed-width layout unlocks O(1) random access.
>
> **What you will build:**
> - A custom `.vec` segment layout with magic bytes, a version field, and a vector body
> - `write_u32`/`write_f32`/`read_u32`/`read_f32` byte-order helpers using Little Endian
> - `write_segment` and `read_segment` functions plus a `read_vector_at` seek-based random-access reader
> - The skill of inspecting raw bytes in a hex editor and decoding IEEE 754 `f32` values

---

## 1. Introduction: Why Not JSON?

In the [previous post](../05-async-axum/index.md), we built an async HTTP server with Tokio and Axum that speaks JSON over the wire. That gave our database a public-facing API; now we turn inward to the storage engine, designing the on-disk format that everything we build later (memory mapping, indexes, the WAL) will read and write.

In that previous post, we built a web server that accepts JSON. It works great for talking to humans and web browsers.
...
- **Mixing byte orders between writer and reader.** If you write with `to_le_bytes()` but read with `from_be_bytes()` (or run on a big-endian target), every number comes back scrambled. Pick one order (we chose Little Endian) and use it on both sides without exception, or your `count` of 1 reads back as 16,777,216.
- **Using `read` instead of `read_exact`.** A plain `read` is allowed to return fewer bytes than you asked for, especially on pipes, sockets, or large files. For fixed-width binary fields you must loop until the buffer is full, which is exactly what `read_exact` does for you. A partial read silently desynchronizes every field after it.
- **Forgetting to validate the magic bytes and version before parsing.** If you skip the `VECT` check, a truncated file, a JPEG, or a v2 file will be interpreted as vector data and produce garbage or a panic. Always reject an unknown signature or version up front rather than trusting the header.
- **Trusting `count` and `dimension` without sanity limits.** A corrupt header that claims `count = 4_000_000_000` will make `Vec::with_capacity(count)` try to allocate gigabytes and abort the process. Treat header fields as untrusted input: validate them against the actual file length before you allocate.
- **Miscomputing the seek offset for random access.** The offset is `16 + (index * dimension * 4)`, and every term must be `u64`. If you compute `index * dimension * 4` in `u32`, it overflows past roughly 4 GB and seeks to the wrong vector. Cast to `u64` first, as `read_vector_at` does.
- **Assuming an `f32` round-trips like a decimal string.** `1.0` is `00 00 80 3F` in IEEE 754, not anything that looks like "1". Do not eyeball float bytes; verify with `f32::to_le_bytes()` or a converter, and remember that values like `0.1` are not exactly representable.

---

## What to read next

- **[Zero-Copy Magic: Memory Mapping with memmap2](../07-mmap/index.md)**: our `read_segment` loads the whole file into RAM; next we map the file into the address space so the OS pages in only the vectors we touch.

---

## Further reading

- The Rust Programming Language, "Reading and Writing Files" and the `std::io` traits (`Read`, `Write`, `Seek`). https://doc.rust-lang.org/book/
- `std::primitive::f32` and the `to_le_bytes`/`from_le_bytes` documentation, on byte-order conversions and IEEE 754 representation. https://doc.rust-lang.org/std/primitive.f32.html
- The `byteorder` crate, for ergonomic endian-aware reads and writes over any `Read`/`Write`. https://docs.rs/byteorder/
- IEEE 754-2019, Standard for Floating-Point Arithmetic, the source of the `f32`/`f64` bit layouts. https://standards.ieee.org/ieee/754/6210/
