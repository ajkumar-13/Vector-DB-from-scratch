...
> **TL;DR.** A single-threaded storage engine cannot serve concurrent HTTP requests, and wrapping it in a plain `Mutex` serializes every operation. This post wraps `VectorStore` in `Arc<RwLock<...>>` so thousands of readers can search in parallel while writers and background compaction take exclusive access when they need it. You will wire that shared state into an Axum server, learn why `tokio::sync` locks beat `std::sync` locks inside async handlers, and avoid the classic upgrade and lock-ordering deadlocks.
>
> **What you will build:**
> - A `SharedVectorStore = Arc<RwLock<VectorStore>>` type and the initialization that hands clones to each consumer.
> - Thread-safe Axum handlers for search (read lock), insert and delete (write lock).
> - A background compaction task plus graceful shutdown that flushes the WAL before exit.
> - Deadlock-safe patterns (drop-before-upgrade, lock ordering, no lock across `.await`) and stress and `loom` tests.

---

## 1. Introduction: The Single-Threaded Bottleneck

Coming out of [Crash Recovery](../09-crash-recovery/index.md), we have a durable storage engine that replays its log and restores state on startup, but every operation runs on one thread. This post is where that engine becomes a real concurrent server, the last piece of the storage layer before we turn to the math and the search index.

In [Post #9](../09-crash-recovery/index.md), we built a complete storage engine with crash recovery. It works, it is durable, and it is fast.
...
Let us update our server from [Post #5](../05-async-axum/index.md).
...
In [Post #9](../09-crash-recovery/index.md), we ran compaction manually. Now we spawn a background task that runs continuously.
...
- **Holding a read lock while requesting a write lock on the same `RwLock`.** This is the upgrade deadlock from section 6.1: the write request waits for the read guard to drop, but the read guard cannot drop until the function returns. Scope the read to a small block, drop it, then take the write lock and re-check (the TOCTOU re-check guards against another writer slipping in).
- **Using `std::sync::RwLock` inside an async handler.** A standard-library guard blocks the OS thread, so a slow search starves every other task scheduled on that Tokio worker. Use `tokio::sync::RwLock` and `.await` the lock whenever the critical section can take milliseconds or contains an `.await`; reserve `std::sync` for sub-microsecond, await-free sections like reading a counter.
- **Keeping a lock alive across an `.await`.** Holding a guard while you call an external API or do I/O serializes the whole database for the duration of that call, and can deadlock if the awaited work needs the same lock. Compute the value you need inside a block, drop the guard, then await. The borrow checker will not save you here, the guard is `Send` and compiles fine.
- **Acquiring multiple locks in different orders across tasks.** If one task takes A then B while another takes B then A, they can deadlock. Pick a single global lock-acquisition order and follow it everywhere.
- **Calling `std::process::exit()` without flushing first.** `exit()` skips destructors, so a `BufWriter` in your WAL never runs its `Drop`-based `flush()` and you silently lose buffered writes. In the shutdown path, explicitly `compact()` and `flush_wal()` while holding the write lock, then exit.
- **Cloning the data instead of the `Arc`.** `Arc::clone(&shared)` bumps a reference count and shares one `VectorStore`; accidentally cloning the inner value (or calling `.clone()` on a dereferenced store) gives each task its own copy, so inserts on one are invisible to the others.

---

## What to read next

- **[Vector Math for Developers](../11-vector-math/index.md)**: with concurrency solved, we open Phase 3 and replace hand-waving with real linear algebra, dot products and cosine similarity, the foundation for fast search.

---

## Further reading

- The Rust Programming Language, "Shared-State Concurrency" and "Fearless Concurrency" chapters, on `Arc<Mutex<T>>` and `RwLock` ownership. https://doc.rust-lang.org/book/ch16-03-shared-state.html (theme: Rust)
- Tokio documentation, `tokio::sync::RwLock` and the "Shared state" tutorial, on async-aware locking and when to hold a lock across `.await`. https://docs.rs/tokio/latest/tokio/sync/struct.RwLock.html (theme: Rust)
- The `loom` crate documentation, on exhaustive interleaving tests for concurrent code. https://docs.rs/loom (theme: Rust)
- `std::sync::RwLock` standard library documentation, on blocking semantics and poisoning. https://doc.rust-lang.org/std/sync/struct.RwLock.html (theme: Rust)
