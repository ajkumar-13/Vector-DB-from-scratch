...
> **TL;DR.** This post turns the HNSW graph you built last time into a working query engine. You will implement the two-phase search (greedy descent through the upper layers, then beam search at Layer 0), learn how the three knobs (`M`, `ef_construction`, `ef_search`) trade memory, build time, latency, and recall, and measure quality with a Recall@K benchmark against brute force. By the end you will have a vector index that beats linear scan by orders of magnitude while holding 98 to 99 percent recall.
>
> **What you will build:**
> - A `search(query, k, ef_search)` method that descends greedily to Layer 1, then beam-searches Layer 0
> - An intuition for tuning `M`, `ef_construction`, and `ef_search`, plus which are fixed at build time and which are runtime-tunable
> - A Recall@K measurement harness that compares HNSW output against an exact brute-force ground truth
> - Benchmarks proving 10x to 75x speedups over brute force as the dataset grows

## 1. Introduction: Navigating the Maze

This is the second half of the HNSW implementation: in [Implementing HNSW Part 1: Building the Graph Structure](../14-hnsw-impl-1/index.md) we built the index structure, and now we add the read path that makes it queryable, completing the search engine that the rest of the series benchmarks and persists.

In [Post #14](../14-hnsw-impl-1/index.md), we built the *structure* of our HNSW index. We have nodes, layers, and edges. We can insert vectors, and the graph magically organizes itself into a hierarchical pyramid.
...
- **Forgetting to clamp `ef_search` up to `k`.** If a caller asks for `k=10` results with `ef_search=5`, the beam search at Layer 0 only ever holds 5 candidates, so you physically cannot return 10. Always do `let ef_search = ef_search.max(k)` at the top of `search()` (as shown), otherwise `truncate(k)` silently returns a short list.
- **Blaming `ef_search` for low recall when the graph is the problem.** Search-time tuning cannot repair a graph built with too few connections. If recall plateaus around 85 percent even at `ef_search=500`, the fix is to rebuild with a larger `M` and `ef_construction`, not to keep cranking the query-time beam. Remember: `Search Quality = f(Graph Quality, ef_search)`, and you cannot fix a bad graph at query time.
- **Greedy-descending all the way to Layer 0.** Stopping greedy descent at Layer 1 is deliberate. A pure single-candidate greedy walk on the dense bottom layer gets trapped in local minima; Layer 0 needs the multi-candidate beam search to escape them. If you let `greedy_descent` run to layer 0 and skip the beam phase, recall collapses on hard queries.
- **Comparing recall without a correct ground truth.** Recall@K is only meaningful if `brute_force_search` returns the true exact top-K with the same distance metric your index uses. Mixing Euclidean ground truth with a cosine-based index, or taking fewer than `k` truth results, produces recall numbers that look fine but are wrong.
- **Treating build time as free.** Around 47 minutes for 1M vectors at `M=16, ef_construction=200` is normal, and it scales with `ef_construction` and `M squared`. Do not raise these blindly chasing the last fraction of a percent of recall; the cost lands entirely at insert time and on memory (roughly `N x M x 1.5 x sizeof(usize)`).
- **Caching results keyed on raw float bytes.** The `CachedHNSW` example hashes the query vector, but two semantically identical queries with tiny floating-point differences hash differently, so the cache misses far more than you expect. Cache only truly identical repeated queries, and never share a mutable cache across threads without synchronization.

---

## What to read next

- **[Benchmarking the Search Engine: Brute Force vs HNSW at Scale](../16-benchmarking/index.md)**: now that search works, we measure it rigorously across dataset sizes, dimensions, and parameters, and chart the full recall-versus-latency trade-off.

---

## Further reading

- Malkov, Y. A., and Yashunin, D. A. *Efficient and robust approximate nearest neighbor search using Hierarchical Navigable Small World graphs.* IEEE TPAMI, 2020. The canonical HNSW paper that defines greedy descent, the `M` / `ef_construction` / `ef_search` parameters, and the recall trade-offs discussed here. https://arxiv.org/abs/1603.09320
- Aumüller, M., Bernhardsson, E., and Faithfull, A. *ANN-Benchmarks: A benchmarking tool for approximate nearest neighbor algorithms.* The standard methodology for measuring Recall@K versus query latency across ANN methods. https://ann-benchmarks.com
- *The Rust Programming Language* (the Rust Book), chapter on common collections, covering `HashSet` and `HashMap` as used in the recall and caching code. https://doc.rust-lang.org/book/ch08-00-common-collections.html
