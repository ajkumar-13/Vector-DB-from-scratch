...
> **TL;DR.** When your database boots, RAM is empty but the disk holds a WAL and a set of segment files. This post teaches you the startup logic that rebuilds in-memory state by loading segments and replaying the log, then implements compaction: the crash-safe move that flushes the MemTable into a permanent segment and truncates the WAL. By the end you have a small LSM-tree-lite store that survives a crash at any point during recovery or compaction.
>
> **What you will build:**
> - An `open()` recovery path that loads `.vec` segments, replays the WAL into a MemTable, and cleans up orphaned `.tmp` files.
> - A crash-safe `compact()` using write-to-temp, `sync_all()`, and atomic rename to turn the log into a read-optimized segment.
> - A `truncate()` method on the WAL and size-, time-, and shutdown-based triggers for when to compact.
> - A mental model of every crash point during compaction and why replay stays idempotent.

---

## 1. Introduction: The Morning After

This post builds directly on [the Write-Ahead Log from Post #8](../08-wal/index.md): now that every `Insert` and `Delete` is durably on disk, we write the startup code that turns that log back into live state and add the compaction step that keeps the log from growing forever.

In [Post #8](../08-wal/index.md), we built a Write-Ahead Log (WAL) that captures every `Insert` and `Delete`. This guarantees that if our server crashes, the data is safe on disk.
...
![Crash Recovery Process: loading segments, replaying the WAL into the MemTable, and compacting back to a new segment](diagrams/recovery-process.svg)
*On startup the store reads cold segments and replays the hot WAL to rebuild state; compaction later folds the MemTable into a fresh segment and clears the log.*
...
| **Memory Access** | WAL uses sequential I/O. We want mmap segments for zero-copy access ([Post #7](../07-mmap/index.md)). |
...
## 5. Step 1: The Replay Logic
 ...
 We already wrote a basic `read_all` in [Post #8](../08-wal/index.md). Now we integrate it into the system startup.
...
**Solution:** We'll fix this in [Post #10](../10-concurrency/index.md) with `RwLock` and snapshot isolation.
...
- **Truncating the WAL before the segment is durable.** If you clear the log before `sync_all()` returns on the temp file, a crash leaves you with an empty WAL and no segment, which is silent data loss. Always order it: write temp, `sync_all()`, atomic rename, then truncate the WAL. The rename is the commit point, so nothing destructive happens before it.

- **Loading `.tmp` files as if they were segments.** A `.tmp` file is a partial write from a compaction that never finished. If `load_segments` picks it up, you mmap garbage and corrupt your read path. Filter strictly on the `.vec` extension when loading, and delete every `.tmp` during `cleanup_temp_files` on startup.

- **Assuming rename guarantees the bytes are on disk.** Rename is atomic for visibility, but it only commits the directory entry. If you skip `file.sync_all()` on the temp file first, a power loss can leave the renamed file pointing at unflushed (zero) blocks. Sync the data, then rename. On some filesystems you also want to fsync the parent directory so the rename itself survives.

- **Treating Scenario B (crash after rename, before truncate) as a bug.** Replaying the WAL on top of an already-flushed segment looks like duplication, but it is harmless because replay is idempotent: last write wins, and the MemTable copy simply shadows the segment copy on reads. Do not add logic to "detect and skip" replayed entries; that complexity is where real corruption bugs hide.

- **Skipping the empty-MemTable check in `compact()`.** Compacting an empty MemTable writes a zero-vector segment and then truncates a WAL that may not need it, growing your segment count for nothing. Return early when `self.memtable.is_empty()`, as the code does, so idle compaction triggers are no-ops.

- **Forgetting that more segments means slower reads.** Every compaction adds a segment, and `get()` scans them newest-first. Without segment merging (major compaction), a long-running store accumulates hundreds of files and read latency creeps up. Plan for a merge strategy before the segment count becomes the bottleneck.

---

## What to read next

- **[Concurrency Control: Arc, RwLock, and Mutex](../10-concurrency/index.md)**: our `compact()` takes `&mut self` and blocks everything; next we make reads run in parallel with compaction using shared ownership and reader-writer locks.

---

## Further reading

- Patrick O'Neil, Edward Cheng, Dieter Gawlick, and Elizabeth O'Neil, "The Log-Structured Merge-Tree (LSM-Tree)," Acta Informatica, 1996. The foundational paper for the MemTable-plus-segments design used here.
- RocksDB documentation, "Compaction" and "WAL" sections (facebook/rocksdb wiki). A production reference for compaction strategies and log truncation.
- The Rust Standard Library documentation for `std::fs::rename` and `std::fs::File::sync_all`, which describe the atomicity and durability guarantees this post relies on.
- The PostgreSQL documentation, "Reliability and the Write-Ahead Log," for the checkpointing terminology and crash-recovery model.
