...
> **TL;DR.** This post turns our in-memory library into a real network service. You will learn how async/await, Futures, and the Tokio runtime let one process juggle thousands of connections without a thread per request, then use Axum to route HTTP requests, parse JSON into your `Vector` struct, and share mutable state across handlers with `Arc<RwLock<T>>`. By the end you have a running server with insert, get, search, stats, JSON error handling, request-logging middleware, and graceful shutdown.
>
> **What you will build:**
> - A production-shaped Axum HTTP server exposing `/health`, `/vectors`, `/vectors/{id}`, `/search`, and `/stats`
> - Thread-safe shared state with `Arc<tokio::sync::RwLock<AppState>>` wired in via `.with_state(...)`
> - A JSON-consistent API error type implementing `IntoResponse`, plus Tower request-tracing middleware
...
This is the transport layer of our build: in the previous post, [Rust Crash Course Part 2: Structs, Enums, and Error Handling](../04-structs-enums-error-handling/index.md), we defined the data structures and error types that the database operates on. Now we put a network boundary in front of them so clients can talk to the engine over HTTP.
...
- **Reaching for `std::sync::RwLock` (or `Mutex`) in handlers.** The std lock blocks the entire OS thread while it waits, which stalls Tokio's scheduler and can deadlock under load. Use `tokio::sync::RwLock` / `tokio::sync::Mutex` whenever a lock might be held across an `.await`, and keep `std` locks only for short, await-free critical sections.
- **Holding a write lock across an `.await`.** Calling `state.write().await` and then awaiting an expensive operation while still holding the guard freezes every other reader and writer. Clone out just the data you need, drop the guard (let it leave scope), do the slow work, then take a fresh short-lived lock to write the result back.
- **Calling blocking code inside an async handler.** `std::thread::sleep`, synchronous file I/O, or a tight CPU loop inside `async fn` parks a runtime worker thread and starves other tasks. Use `tokio::time::sleep(...).await` for delays and `tokio::task::spawn_blocking(...)` for genuinely blocking or CPU-heavy work.
- **Forgetting `async move` when spawning.** `tokio::spawn` requires a `Send + 'static` future, so borrowing a local variable (like a `String` from the surrounding scope) fails to compile. Add `move` to transfer ownership into the task, and remember the value is no longer usable in the outer scope afterward.
- **Returning plain-text errors from a JSON API.** Returning `(StatusCode, String)` gives clients an inconsistent body shape. Define an `ApiError` type that implements `IntoResponse` and always serializes to `{"error": true, "message": ...}` so success and failure responses are uniform.
- **Shipping without graceful shutdown.** A bare `axum::serve(...).await` kills in-flight requests on `Ctrl+C`, which is dangerous once writes touch disk. Wire `.with_graceful_shutdown(shutdown_signal())` so the server stops accepting new connections but lets running requests finish first.
...
- **[Binary File Formats: Designing a Custom Segment Layout](../06-binary-file-formats/index.md)**: Give the database a memory by leaving JSON behind and designing a compact binary segment format that survives restarts.

---

## Further reading

- The Rust Programming Language ("the Book"), Chapter 16 (Fearless Concurrency) and Chapter 17 (Async and Await): the canonical reference for ownership across threads, `Send`/`'static`, and the futures model. https://doc.rust-lang.org/book/
- Tokio documentation and the official Tokio tutorial: the runtime, tasks, `spawn`, and async-aware synchronization primitives like `RwLock`. https://tokio.rs/tokio/tutorial
- Axum crate documentation: routers, extractors (`Json`, `State`, `Path`), `IntoResponse`, and `with_state`. https://docs.rs/axum
- Tower crate documentation: the `Service` and `Layer` abstractions behind Axum's composable middleware (tracing, timeouts, CORS). https://docs.rs/tower
