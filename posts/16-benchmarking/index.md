...
> **TL;DR.** This post stops adding features and starts measuring them, putting our Brute Force and HNSW engines head-to-head across 10K, 100K, and 1M vectors. You will build a proper benchmark harness (warmup, percentiles, recall calculation) and learn to read the Recall vs Latency Pareto frontier so you can pick `ef_search` with data instead of guesswork. The payoff is hard numbers showing where Brute Force dies (around 50K to 100K vectors) and why HNSW becomes mandatory at scale.
>
> **What you will build:**
> - A reusable Rust benchmark harness that reports P50/P95/P99 latency, QPS, recall, build time, and memory
> - Side-by-side measurements of Brute Force vs HNSW at three dataset sizes
> - A Pareto frontier sweep over `ef_search` to find the recall/latency sweet spot
> - A decision matrix and adaptive search strategy for choosing an algorithm by dataset size

## 1. Introduction: The Race

Coming out of [Implementing HNSW: Search Algorithm and Parameter Tuning](../15-hnsw-impl-2/index.md), we now have two complete engines sitting side by side in the project: the Brute Force index from Phase 2 and the fully tuned HNSW graph from Phase 3. This post is the moment of truth where we measure them honestly against each other before moving on to persistence and hybrid search.
...
- **Reporting the average and calling it a day.** Averages hide the spikes that break SLAs. A 2ms mean can sit on top of 50ms P99 tails. Always sort your latencies and report P95 and P99, exactly as `calculate_percentiles` does, because those are the numbers your users actually feel.
- **Forgetting the warmup phase.** Your first queries hit cold CPU caches, an unfilled OS page cache, and an untrained branch predictor, so they look misleadingly slow. Run hundreds of warmup queries before you start the clock, or HNSW's advantage will be understated and your numbers will not reproduce.
- **Benchmarking with one query vector in a loop.** Searching the same vector repeatedly turns the test into a cache-hit benchmark, not a search benchmark. Use diverse queries (ideally real embeddings), and remember that uniform random data is the worst case for HNSW, so real clustered data will only make it look better.
- **Comparing recall against the wrong ground truth.** HNSW recall only means something relative to the exact answer. Compute ground truth with the Brute Force engine on the same dataset and the same `k`, then compare sets. If you skip this, a "99% recall" claim is meaningless.
- **Reading the Pareto curve as linear.** The recall/latency curve flattens hard: going from 98% to 100% recall can cost 4x the latency. Do not chase the last percent of recall by cranking `ef_search`; pick the knee of the curve (around `ef_search` = 100 here) unless your use case truly demands it.
- **Assuming the benchmark is the whole story.** These single-threaded numbers ignore concurrency, where Brute Force is memory-bandwidth bound and degrades fast under load while HNSW scales with cores. They also ignore that HNSW lives entirely in RAM and must be rebuilt on restart, the exact flaw the next posts address.

---

## What to read next

- **[Inverted Indexes Explained: How Text Search and Keyword Filtering Actually Work](../17-inverted-indexes/index.md)**: We leave pure vector search behind and build the inverted index that powers exact-match and keyword filtering, the first half of hybrid search.

---

## Further reading

- Malkov, Y. A., and Yashunin, D. A. "Efficient and robust approximate nearest neighbor search using Hierarchical Navigable Small World graphs." IEEE TPAMI, 2020. The HNSW paper, including the role of `ef` in the speed/recall trade-off. https://arxiv.org/abs/1603.09320
- Aumüller, M., Bernhardsson, E., and Faithfull, A. "ANN-Benchmarks: A Benchmarking Tool for Approximate Nearest Neighbor Algorithms." Information Systems, 2020. The standard methodology for recall vs queries-per-second comparisons. https://ann-benchmarks.com/
- The Criterion.rs documentation: statistics-driven benchmarking in Rust, covering warmup, outlier detection, and percentile reporting. https://bheisler.github.io/criterion.rs/book/
- The Rust Performance Book: practical guidance on measuring and profiling Rust programs, including allocation profiling. https://nnethercote.github.io/perf-book/
