...
> **TL;DR.** This post teaches you to model data with `struct`, model state with `enum`, and handle failure with `Option` and `Result` instead of nulls and exceptions. You will use traits (`Display`, `From`) to wire up clean error conversion via the `?` operator. By the end you will have written the real `vectordb::models` module: the typed, tested foundation every later phase builds on.
>
> **What you will build:**
> - The core `Vector` type with a constructor that enforces invariants, plus `magnitude` and `normalize` methods.
> - A `DistanceMetric` enum (Cosine, Euclidean, Dot) with a `calculate` method driven by exhaustive `match`.
> - A `VectorDbError` enum with `Display` and `From` impls so `?` propagates and converts errors automatically.
> - The wired-up `src/models.rs` and `src/lib.rs` for the `vectordb` project, verified by `cargo test`.

## 1. Introduction: Modeling the World

In the [previous post](../03-ownership-borrowing-memory/index.md) we mastered memory: ownership, borrowing, and moves. Now we use those rules to build the actual vocabulary of the database, the typed `Vector`, `DistanceMetric`, and error types that every later phase (storage, the HNSW index, the HTTP layer) will import.
...
- **Trusting a redundant `dimension` field.** If you store `dimension` separately from `data` (as in the very first `Vector` draft) and set it by hand, nothing stops `dimension: 5` from sitting next to a 3-element `Vec`. Compute it in the constructor (`data.len()`) or expose it as a method, so the invariant cannot drift.
- **Expecting `?` to convert error types for free.** The `?` operator only compiles when the function's error type implements `From` for the source error. `File::open(...)?` inside a function returning `Result<(), VectorDbError>` fails until you add `impl From<std::io::Error> for VectorDbError`. Reach for `map_err` only as a stopgap; the `From` impl is the real fix.
- **Forgetting to implement `Display` (and `std::error::Error`) on your error enum.** `#[derive(Debug)]` gives you `{:?}`, but callers, logs, and the `?` chain expect a human-readable `{}`. Without `impl fmt::Display` and `impl std::error::Error`, your error will not behave like a real error type in idiomatic code.
- **Reaching for `unwrap()` in library or production paths.** `unwrap()` and `expect()` panic on `None`/`Err`, which is fine in a test or a quick demo but turns a recoverable "vector not found" into a process crash in real code. Return `Option`/`Result` and let the caller decide.
- **Writing a non-exhaustive `match` and patching it with `_ =>`.** A catch-all arm silences the compiler today but hides the bug tomorrow: add a new `DistanceMetric` variant and the missing logic compiles silently. Match every variant explicitly so the compiler forces you to handle additions.
- **Adding Serde derives too late.** `SearchRequest`, `Vector`, and friends get serialized in the Axum layer next post. If you skip `#[derive(Serialize, Deserialize)]` now, you will revisit every type later. Derive them up front, as `models.rs` already does.

---

## What to read next

- **[The Async Runtime and HTTP Layer with Axum](../05-async-axum/index.md)**: take the typed models you just built and expose them over a real async HTTP server using Tokio, futures, and Axum.

---

## Further reading

- The Rust Programming Language ("The Book"), chapters on Structs, Enums and Pattern Matching, and Error Handling. The canonical, free reference for everything in this post.
- The Rust standard library docs for [`Option`](https://doc.rust-lang.org/std/option/enum.Option.html), [`Result`](https://doc.rust-lang.org/std/result/enum.Result.html), and the [`From`](https://doc.rust-lang.org/std/convert/trait.From.html) trait.
- The `thiserror` crate, which derives `Display`, `Error`, and `From` for error enums so you can skip the boilerplate shown here once you understand it.
- Tony Hoare, "Null References: The Billion Dollar Mistake" (QCon London, 2009), the talk behind why Rust chose `Option<T>` over null.
