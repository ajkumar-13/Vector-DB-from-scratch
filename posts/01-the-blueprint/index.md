...
> **TL;DR.** This post lays out the architecture for a single-node, persistent vector database you will build over the next 20 posts in Rust. You will learn what vectors and embeddings actually are, how semantic search beats keyword matching, and how the three layers (Transport, Core Engine, Storage) fit together. We also lock down the HTTP API contract and explain why Rust is the right language for the job.
>
> **What you will build:**
> - A mental model of the full system: HTTP transport, query-planning core engine, and a WAL-plus-segments storage layer
> - The complete REST API surface (create collection, upsert, search, delete) you will implement in later posts
> - An intuition for vectors, embeddings, and cosine similarity, plus a runnable cosine-similarity preview
> - A concrete roadmap mapping each upcoming post to a piece of the database

---

## 1. Introduction: The "Zero to Master" Promise

This is the opening post of a 20-part journey: by the final post you will have built a working, crash-safe, HTTP-served vector database in Rust, with no prior Rust knowledge assumed. Think of this post as the blueprint you will return to as each later post fills in a layer.
...
- **Treating "vector" as something mystical.** A vector is just a list of numbers (often 768 or 1536 of them). The meaning lives in the embedding model that produced it, not in the storage format. If you keep this straight, the rest of the system is plumbing.
- **Assuming cosine similarity is always the right distance.** The API exposes `cosine`, `euclidean`, and `dot` for a reason: the correct choice depends on whether your embedding model normalizes its outputs. Pick the distance your model was trained for, or your "nearest" neighbors will be subtly wrong.
- **Forgetting to validate that both vectors share a dimension.** As the preview `cosine_similarity` shows, Rust's `zip()` silently truncates to the shorter slice, so mismatched dimensions produce a plausible-looking but meaningless score instead of an error. Always check `a.len() == b.len()` (we return a `Result` for this in Post #11).
- **Believing Rust beats Python purely on math speed.** NumPy's `dot()` is already near-optimal C. Rust's real edge here is the absence of a GIL, no garbage-collection pauses, and zero glue-code overhead across networking, query planning, and math. Sell Rust on predictable latency under concurrency, not on a microbenchmark.
- **Skipping the Write-Ahead Log because "it works on my machine."** Without a WAL, a crash mid-write corrupts your data silently. The append-then-compact-to-segments design is what lets the database recover by replaying the log on startup. Build it early (Post #8) rather than bolting durability on later.
- **Using HTTP DELETE with a request body for bulk deletes.** The HTTP spec discourages bodies on DELETE and some proxies strip them, so a delete that works locally can vanish in production. Use `POST .../delete` with the IDs in the body, as the API design does.

---

## What to read next

- **[Setting Up the Forge](../02-setting-up-the-forge/index.md)**: install the Rust toolchain, lay out the Cargo project structure, and write your first program so we can stop theorizing and start building.

---

## Further reading

- *The Rust Programming Language* (the official "Rust Book"), the canonical introduction to ownership, the borrow checker, and `Result`/`Option`: https://doc.rust-lang.org/book/
- Tokio documentation, the async runtime that powers the Transport Layer: https://tokio.rs/
- Axum documentation, the web framework we use for the HTTP API: https://docs.rs/axum/
- Mikolov et al., "Efficient Estimation of Word Representations in Vector Space" (Word2Vec), the source of the king/queen embedding-geometry example: https://arxiv.org/abs/1301.3781
