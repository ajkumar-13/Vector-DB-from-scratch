...
> **TL;DR.** Vector search is fuzzy: it finds conceptually similar items but ignores hard constraints like `price < 100`. This post builds the missing half of a search engine, the inverted index, a term-to-documents data structure that turns keyword and tag filtering into O(1) lookups plus fast sorted-list set operations. You will implement tokenization, postings lists, and Boolean AND/OR/NOT queries in Rust, then see how to combine them with HNSW for hybrid search.
>
> **What you will build:**
> - An `InvertedIndex` struct backed by a `HashMap<String, Vec<usize>>` with document insertion and tokenization
> - A two-pointer intersection algorithm that merges sorted postings lists in O(n + m) time
> - Boolean query methods (`search_and`, `search_or`) with shortest-list-first optimization
> - A clear picture of pre-filtering vs post-filtering for hybrid vector plus keyword search

---

## 1. Introduction: The Limits of Vector Search

Coming off the last post, [Benchmarking the Search Engine: Brute Force vs HNSW Performance Comparison at Scale](../16-benchmarking/index.md), we have a measured, fast HNSW engine and the numbers to prove it. But raw speed on similarity is only half a search engine: this post adds the keyword and filtering half, the inverted index, which we will fuse with HNSW for hybrid search in the next phase.
...
- **Forgetting to keep postings lists sorted and deduplicated.** The `add_document` shortcut only checks `list.last() != Some(&doc_id)`, which assumes documents arrive in increasing DocID order. If you index out of order or re-index a document, the two-pointer `intersect` silently produces wrong results because it relies on sorted input. Call `optimize()` (sort_unstable + dedup) before querying any index that was not built strictly in order.
- **Building a HashSet just to intersect two sorted lists.** It is tempting to write `set_a.intersection(&set_b)`, but you pay O(n) allocation to build the set and lose cache locality. For already-sorted postings lists, the two-pointer merge is O(n + m) time with O(1) extra space and sequential memory access. Reach for `RoaringBitmap` only when lists are large (> 10K) or set operations dominate.
- **Intersecting AND terms in the wrong order.** `search_and` sorts candidate lists by length and starts with the shortest for a reason: intersecting the 1M-element "shoes" list first wastes work. Skip that sort and a query like `shoes AND nike AND blue` does millions of comparisons instead of hundreds. Always intersect smallest-first and bail out the moment the running result is empty.
- **Confusing post-filtering with pre-filtering in hybrid search.** Post-filtering (search HNSW, then drop non-matches) is easy but produces false negatives: if none of the top-K vectors match your filter, you return zero results even when thousands of matches exist. Pre-filtering (compute the allow-list first, then constrain HNSW traversal) avoids this but needs a fast membership test, a bitmask or HashSet, woven into the graph walk.
- **Tokenizing without normalization or stemming.** The toy `tokenize()` lowercases and strips punctuation but does nothing else, so "running" never matches "run" and accented terms miss entirely. Without stemming and unicode normalization your recall quietly drops on real text. Plug in `rust-stemmers` and normalize before you ship, or expect users to complain that obvious matches are missing.
- **Treating the index as thread-safe.** A bare `HashMap` cannot be shared across query threads. Wrapping it in `RwLock` serializes all writers and `.clone()`s postings on every read; for concurrent workloads prefer `DashMap` or an immutable, atomically swapped index so reads never block.

---

## What to read next

- **[The Hybrid Engine: Integrating Tantivy for High-Speed Metadata Filtering](../18-tantivy-hybrid/index.md)**: We swap our toy index for production-grade Tantivy and wire bitmask-guided pre-filtering into HNSW to answer "running shoes under $100" correctly and fast.

---

## Further reading

- **Tantivy** (the full-text search engine library we adopt next): documentation at https://docs.rs/tantivy and source at https://github.com/quickwit-oss/tantivy.
- **Apache Lucene**, the canonical inverted-index engine whose postings, segments, and codec design inspired most of this post: https://lucene.apache.org/core/.
- **RoaringBitmap**, the compressed bitmap structure for fast set operations on DocID lists, with the reference implementation and paper links at https://roaringbitmap.org/ and the Rust crate at https://docs.rs/roaring.
- **The `rust-stemmers` crate** for Snowball/Porter stemming during tokenization: https://docs.rs/rust-stemmers.
