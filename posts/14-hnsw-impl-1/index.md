...
> **TL;DR.** This post turns HNSW theory into running Rust by building the in-memory graph structure from scratch. You will design an arena-based graph layout, implement single-layer beam search, generate probabilistic layer assignments, and write the two-phase insertion algorithm with a diversity heuristic and edge pruning. By the end you have a working `HNSWIndex` that ingests vectors and builds a navigable multi-layer graph.
>
> **What you will build:**
> - An arena-backed `HNSWIndex` and `Node` struct using `usize` indices instead of pointers.
> - A `search_layer` beam search over a single layer using two binary heaps.
> - Probabilistic level generation with exponential decay, plus the two-phase insertion routine.
> - The neighbor selection diversity heuristic and edge pruning to bound connections at M.

---

## 1. Introduction: From Theory to Code

This is the second stop on the HNSW leg of our build: in the previous post, [Introduction to HNSW](../13-hnsw-intro/index.md), we covered the theory (skip lists, small worlds, greedy search), and now we put that theory into Rust by constructing the graph that every later search, persistence, and concurrency post will build on top of.

In [Post #13](../13-hnsw-intro/index.md), we explored the theory behind **HNSW** (Hierarchical Navigable Small World). We learned about:
...
- **Forgetting bidirectional edges.** It is easy to add `new_node to neighbor` and forget the reverse `neighbor to new_node` link. HNSW is an undirected graph: if you only link one direction, the neighbor never knows about the new node, search cannot route through it, and recall quietly collapses. Always call `connect` on both endpoints at the same layer, as the insertion loop does.
- **Indexing connections by absolute layer when a node is short.** `connections[layer]` only exists up to a node's own `layer_count`. If you index a layer above a node's height (for example reading layer 3 on a node that only reaches layer 1), you panic or read garbage. The `neighbors` helper guards this by returning an empty slice for out-of-range layers; reuse it instead of indexing `connections` directly.
- **Comparing raw f32 in a `BinaryHeap`.** `f32` is not `Ord` because of NaN, so you cannot put it in a heap without a wrapper. The `OrderedFloat` newtype exists for exactly this reason. Skipping it forces `unwrap()` on `partial_cmp` in hot loops, and a single NaN distance (often from an un-normalized or zero vector) will then crash the whole build.
- **Mixing up the two heaps in `search_layer`.** `candidates` is a min-heap (explore closest first, hence the `Reverse` wrapper) and `results` is a max-heap (so `peek` gives the worst kept result to evict). Swapping their orderings makes the early-termination check (`current_dist > worst_dist`) wrong, so search either stops too early (low recall) or never prunes (slow).
- **Pruning with naive closest-M instead of the diversity heuristic.** When a neighbor exceeds its M limit, trimming to the M closest connections re-creates the clustering problem the heuristic was built to avoid. `prune_connections` deliberately reuses `select_neighbors_heuristic` so connections stay spread across directions. If you shortcut this, long-range navigability degrades after many inserts.
- **Skipping the entry-point update when a taller node arrives.** A new node with a level above `max_layers` must become the entry point, and `max_layers` must be raised. Forget this and Phase 1 keeps zooming in from a node that no longer reaches the top layers, so the upper levels of the new node are never used for routing.

---

## What to read next

- **[Search Algorithm and Parameter Tuning](../15-hnsw-impl-2/index.md)**: We add the query path on top of this graph, then tune M, ef_construction, and ef_search to trade recall against latency and benchmark the result.

---

## Further reading

- Yu. A. Malkov and D. A. Yashunin, "Efficient and robust approximate nearest neighbor search using Hierarchical Navigable Small World graphs," IEEE TPAMI, 2018. The canonical HNSW paper, source of the insertion algorithm, layer probability, and neighbor selection heuristics implemented here. https://arxiv.org/abs/1603.09320
- The Rust Programming Language (the Rust Book), chapter on collections (`Vec`, `HashSet`) and ownership, which underpins the arena pattern used to avoid pointer-based graphs. https://doc.rust-lang.org/book/
- The `rand` crate documentation, used for the random level generation in `random_level`. https://docs.rs/rand/
