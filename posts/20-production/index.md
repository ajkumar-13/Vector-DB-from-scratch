...
> **TL;DR.** This final post turns the prototype engine into a production system. You compress vectors 4x with scalar quantization, profile the hot path with flamegraphs and hand-tune the distance kernel with AVX2 SIMD, then package everything in a tiny multi-stage Docker image with a GitHub Actions CI/CD pipeline. It closes by reviewing the full architecture built across the series.
>
> **What you will build:**
> - A `QuantizedVector` type that maps f32 to u8 for a 4:1 memory reduction with roughly 1.4% recall loss
> - A SIMD (AVX2) dot product with runtime feature detection and a scalar fallback
> - A 58 MB multi-stage Dockerfile using cargo-chef for dependency caching
> - A CI/CD workflow that gates merges on format, lint, tests, benchmarks, and image builds

## 1. Introduction: "It Works on My Machine"

This is the last stop in the build. In the previous post, [Query Planning: Designing an Optimizer to Choose Between Vector-First and Filter-First Execution](../19-query-planning/index.md), you taught the engine to pick an execution strategy based on selectivity; now you take that fully-featured engine and make it cheap to run, fast on real CPUs, and deployable. By the end you will have a containerized, profiled, CI-gated database instead of a research prototype.
...
- **Quantizing low-dimensional vectors and expecting clean recall.** Scalar quantization leans on the blessing of dimensionality: per-dimension rounding errors average out across hundreds of components. On vectors under roughly 32 dimensions, or distributions clustered near zero, the same error swamps the signal. Keep f32 for low-dimensional data, or use the hybrid re-ranking mode.
- **Trusting quantized distances as exact.** `approx_distance` is for fast candidate generation, not final scoring. If you sort and return top-k purely on u8 math, you will occasionally misorder near-ties. Re-rank the top candidates against the original f32 vectors before responding.
- **Reading a flamegraph's X-axis as time.** The horizontal axis is alphabetical stack ordering, not a timeline. Width means proportion of CPU samples; a wide bar is a hot function regardless of where it sits left to right. Optimizing the leftmost bar because it "runs first" is wasted effort.
- **Calling AVX2 intrinsics without a runtime guard.** A binary built with `target_feature = "avx2"` that runs `dot_product_avx2` unconditionally will SIGILL on a CPU lacking AVX2. Always gate on `is_x86_feature_detected!("avx2")` and fall back to the scalar path, and handle the non-multiple-of-8 tail explicitly.
- **Skipping cargo-chef and rebuilding dependencies every Docker build.** Copying the full source before `cargo build` invalidates the dependency layer on any code change, so every build recompiles all crates. Prepare a recipe from just `Cargo.toml`/`Cargo.lock` first, `cargo chef cook` the deps into a cached layer, then copy source. This is the difference between a 30-second and a 10-minute image build.
- **Letting the benchmark job pass when it has no baseline.** A regression check that silently succeeds when `benches/baseline.json` is missing gives false confidence. Commit a real baseline, fail loudly if it is absent, and update it deliberately when a change is genuinely faster, never automatically.

---

## What to read next

This is the end of the road, so the best next step is to loop back to where it started and re-read the design with implementer's eyes, then extend the system.

- **[The Blueprint](../01-the-blueprint/index.md)**: revisit the opening architecture now that every box in the diagram is something you have actually built.

Concrete extensions to take on next:

- **Product Quantization:** replace the linear u8 mapping in `QuantizedVector` with K-Means codebooks for 32x compression on datasets past 100M vectors.
- **Raft replication:** add the `raft` crate on top of the WAL so the log replicates to followers, giving you crash-safe multi-node durability.
- **Observability:** export the Prometheus metrics already scaffolded in the API layer and wire them into the Grafana service from the Docker Compose file.

---

## Further reading

- The Rust Performance Book, Nicholas Nethercote and contributors: practical guidance on cloning, bounds checks, and allocation in hot paths.
- The `std::arch` module documentation in the Rust standard library: the canonical reference for `is_x86_feature_detected!`, `#[target_feature]`, and the AVX2 intrinsics used here.
- "Efficient and Robust Approximate Nearest Neighbor Search Using Hierarchical Navigable Small World Graphs", Malkov and Yashunin (2018): the HNSW index this database is built on.
- "Product Quantization for Nearest Neighbor Search", Jegou, Douze, and Schmid (2011): the basis for the higher-compression quantization extension.
