...
> **TL;DR.** Sorting a million similarity scores just to keep the top 10 wastes most of the work. This post shows why a binary heap is the right tool for Top-K retrieval, how the array layout makes it cache-friendly, and the counterintuitive trick of using a Min-Heap to track the *Maximum* scores. You will end with a reusable `O(N log k)` Rust `top_k` function built on `BinaryHeap` and `Reverse`.
>
> **What you will build:**
> - A bounded `top_k` selection routine using `std::collections::BinaryHeap` and `std::cmp::Reverse`
> - The mental model for "Min-Heap as a bouncer" that evicts the weakest of the best in O(1)
> - An intuition for the array-backed heap layout (parent/child index math) and why it beats pointer trees
> - A clear sense of when heaps win over a full sort, backed by complexity and benchmark numbers

---

## 1. Introduction: The Sorting Bottleneck

This deep dive picks up right where [the brute force engine](../12-brute-force/index.md) left off: that post computed an exact similarity score for every vector in the database, and now we need to select the best k of those scores without paying to sort all of them. The heap built here becomes the selection layer that sits on top of every search engine we write from here on, including the HNSW graph traversal in the next post.

In our Vector Database, every search query asks the same question: *What are the k vectors most similar to this query?*

In [Post #12](../12-brute-force/index.md), we implemented Brute Force search. We calculated the similarity score for every single vector in the database.
...
In [Post #13](../13-hnsw-intro/index.md), we will use heaps for **beam search** in the HNSW graph.
...
In [Post #13](../13-hnsw-intro/index.md), we will see heaps again in the context of **HNSW graph traversal**, where they maintain the beam of candidate nodes during approximate nearest neighbor search.
...
- **Reaching for a Max-Heap to find the maximum.** It feels natural, but a Max-Heap puts the *best* item at the root, so deciding whether to evict forces you to scan all k members for the weakest. Use a Min-Heap: the root is the worst of the current best, which is exactly the eviction candidate you want in O(1).
- **Forgetting that `BinaryHeap` is a Max-Heap by default.** If you push raw `Candidate` values and never wrap them in `std::cmp::Reverse`, you build the wrong heap and silently keep the lowest scores. Wrap on push and unwrap on collect, as in the `top_k` example.
- **Implementing `Ord` with a naked `partial_cmp(...).unwrap()`.** Scores are `f32`, and any `NaN` (from a bad distance computation or an empty vector) makes `partial_cmp` return `None`, panicking your whole query. Sanitize or filter `NaN` scores before they reach the heap, or map them to a defined ordering instead of unwrapping blindly.
- **Pushing every candidate, then truncating.** Calling `heap.push` for all N items and only trimming at the end makes the heap grow to size N, which costs O(N log N) and O(N) memory, erasing the win. Keep the heap bounded at k: push only while it has room, and otherwise compare against the root first.
- **Sorting before checking the bouncer.** The early-reject branch (`item.score > min_item.score`) is where most of the speedup comes from. Skipping it and always doing pop/push throws away the "reject without touching the heap" optimization that makes the real-world numbers beat the theoretical 6x.
- **Confusing heap order with sorted output.** `heap.into_iter()` yields the k winners in no particular order, not sorted. If you need a ranked list, sort just those k results afterward (the hybrid approach from section 9.3), which is cheap because k is small.

---

## What to read next

- **[Introduction to HNSW: approximate nearest neighbors](../13-hnsw-intro/index.md)**: We trade exact brute force for a graph-based index, and the heap you just built returns as the beam that tracks candidate nodes during traversal.

---

## Further reading

- *The Rust Programming Language* (the Rust Book), Klabnik and Nichols, no starch press: chapters on traits, generics, and ordering that back the `Ord`/`Eq`/`Reverse` work here. https://doc.rust-lang.org/book/
- `std::collections::BinaryHeap` standard library documentation, The Rust Project: the canonical reference for the Max-Heap behavior and the `Reverse` wrapper used in this post. https://doc.rust-lang.org/std/collections/struct.BinaryHeap.html
- *Introduction to Algorithms*, Cormen, Leiserson, Rivest, and Stein (CLRS), MIT Press: the binary heap and priority queue chapter, including the array index math and heapify analysis.
- *Algorithms*, Sedgewick and Wayne, Addison-Wesley: priority queues and heap-based selection, with clear treatment of when heap selection beats a full sort.
